...
def test_hooks_default_hook_no_context_help(chdir, capsys):
...
def test_hooks_default_hook_no_context_method_call_help(chdir, capsys):
...
def test_hooks_cli_hook_arg_help(chdir, capsys):
...
def test_hooks_cli_hook_arg_help_no_arg(chdir, capsys):
    """Validate that we can run a hook's with an arg."""
    chdir('cli-fixtures')
    with pytest.raises(SystemExit):
...
def test_hooks_cli_no_default_hook(chdir, capsys):
...
def test_hooks_cli_tackle_help_no_arg(chdir):
...
def test_hooks_cli_tackle_default_unknown(chdir, capsys):
    """
...
def test_hooks_cli_default_hook_embedded(chdir, capsys):
...
def test_hooks_import_func_from_hooks_dir_method_no_help(chdir, capsys):
    """
