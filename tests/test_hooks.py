...
from cookiecutter import hook, utils, exceptions
...
class TestFindHooks(object):
    ...
    def test_find_hook(self):
        ...
        with utils.work_in(self.repo_path):
            ...
            actual_hook_path = hook.find_hook('pre_gen_project')
            assert expected_pre == actual_hook_path

            expected_post = os.path.abspath('hooks/{}'.format(self.post_hook))
            actual_hook_path = hook.find_hook('post_gen_project')
    ...
    def test_no_hooks(self):
        ...
        with utils.work_in('tests/fake-repo'):
            assert None is hook.find_hook('pre_gen_project')

    def test_unknown_hooks_dir(self):
        """`find_hooks` should return None if hook directory not found."""
        with utils.work_in(self.repo_path):
            assert hook.find_hook('pre_gen_project', hooks_dir='hooks_dir') is None

    def test_hook_not_found(self):
        """`find_hooks` should return None if the hook could not be found."""
        with utils.work_in(self.repo_path):
            assert hook.find_hook('unknown_hook') is None
...
class TestExternalHooks(object):
    ...
    def test_run_script(self):
        ...
        hook.run_script(os.path.join(self.hooks_path, self.post_hook))
        assert os.path.isfile('shell_post.txt')

    def test_run_script_cwd(self):
        """Change directory before running hook."""
        hook.run_script(os.path.join(self.hooks_path, self.post_hook), 'tests')
    ...
    def test_run_hook(self):
        ...
        with utils.work_in(self.repo_path):
            hook.run_hook('pre_gen_project', tests_dir, {})
            assert os.path.isfile(os.path.join(tests_dir, 'python_pre.txt'))

            hook.run_hook('post_gen_project', tests_dir, {})
...
def test_ignore_hook_backup_files(monkeypatch, dir_with_hooks):
    ...
    assert hook.find_hook('pre_gen_project') is None
    assert hook.find_hook('post_gen_project') is None
