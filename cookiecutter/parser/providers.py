...
from cookiecutter.utils.paths import listdir_absolute
from cookiecutter.repository import is_git_repo

from typing import TYPE_CHECKING
from cookiecutter.models import Provider, BaseHook

if TYPE_CHECKING:
    from cookiecutter.models import Context, Source, Mode, Settings

logger = logging.getLogger(__name__)


def get_providers(
    context: 'Context', s: 'Source', settings: 'Settings', mode: 'Mode'
) -> ['Provider']:
    """
    ...
    if len(context.providers) == 0:
        append_provider_dicts(native_providers, context, settings, mode)

    # Get provider dirs
    if settings.extra_providers:
        # Providers from config file
        append_provider_dicts(settings.extra_providers, context, settings, mode)

    if '__providers' in context.input_dict[context.context_key]:
        append_provider_dicts(
            context.input_dict[context.context_key]['__providers'],
            context,
            settings,
            mode,
        )
...
def append_provider_dicts(
    input_providers, context: 'Context', settings: 'Settings', mode: 'Mode'
):
    ...
    for i in input_providers:
        if isinstance(i, str):
            ...
            if os.path.isfile(hooks_init_path):
                ...
                try:
                    p = Provider(
                        path=hooks_path, hook_types=mod.hook_types, name=mod_name
                    )
                    logger.debug(
