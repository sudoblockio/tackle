...
from tackle.models import BaseHook, import_from_path
...
class ImportHook(BaseHook):
    ...
    def execute(self) -> None:
        if isinstance(self.src, str):
            ...
            import_from_path(self.provider_hooks, provider_path)

        elif isinstance(self.src, list):
            for i in self.src:
                if isinstance(i, str):
                    import_from_path(
                        provider_hook_dict=self.provider_hooks,
                        provider_path=self.get_dir_or_repo(i, None),
                    )
            ...
            import_from_path(
                provider_hook_dict=self.provider_hooks, provider_path=provider_dir
            )
