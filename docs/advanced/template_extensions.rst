...
To do so, a template author must specify the required extensions in ``tackle.yaml`` as follows:
...
installing the `jinja2_time.TimeExtension`_ and enabling it in ``tackle.yaml``.
